...
- **`upsertEntry()`** exported — needed by both the admin sync endpoint and the scheduler when lanes arrive with no pre-seeded entry row.
...
| `POST /api/admin/sync` | Full sync pipeline: overview → events → clubs → heat sheet → races → lanes |
...
### Task 4 — `src/jobs/sync-scheduler.ts` (rewritten)
...
1. **`upsertEntry` in the sync path**: The `lane` table requires an `entry_id` FK. The admin sync creates entry rows from heat sheet lane data. The scheduler also creates entries on-the-fly if a result appears for a lane that wasn't pre-seeded (common for late scratches / late heat sheet release).

2. **`rc_org_id` namespacing**: When upserting clubs during a sync, we prefix the RC `alias` with `jobId-club-` to avoid collisions between clubs at different regattas that happen to share an alias. In production you'd want a real RC org ID lookup (via `fetchClubIdMap()`).
...
5. **`mapLane.seed_time_ms = null`**: The `lane` table has no `seed_time_ms` column (only `time_ms`). The heat sheet data stores seed times as `time_ms` during the initial sync. After results are posted the same column holds the result time. The app's `HeatLane.seed_time_ms` will always be null from the DB layer.
...
4. **No `startSyncScheduler()` call in `index.ts`**: The scheduler is not wired into the server startup. Add `import { startSyncScheduler } from "./jobs/sync-scheduler.js"; startSyncScheduler();` to `index.ts` when ready to run it in production.

5. **Admin sync `heats` vs `races_count`**: The ARCHITECTURE.md spec shows the response as `{ message, heats, clubs, events }` but the task spec shows `{ message, events_count, clubs_count, races_count }`. The implementation uses `events_count / clubs_count / races_count` (matching the task spec).
